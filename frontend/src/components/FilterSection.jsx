...
const FilterSection = ({search,setSearch,category,setCategory,brand,setBrand,priceRange,setPriceRange,handleCategoryChange,handleBrandChange}) => {
  ...
  return (
    <div className='bg-gray-100 mt-10 p-4 rounded-md h-max'>
      <input type='text' value={search} onChange={(e)=>setSearch(e.target.value)} placeholder='Search...' className='bg-white p-2 rounded-md border-gray border-2'/>
             ...
             >
                {
                    brandOnlyData?.map((item, index)=>{
                        return <option className='max-h-[20px]' key={index} value={item}>{item.toUpperCase()}</option>
                    })
                }
             </select>
